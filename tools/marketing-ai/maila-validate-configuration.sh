...
if [[ "$SKIP_K8S" == false ]]; then
  ...
  if [[ -n "$NAMESPACE" ]]; then
    ...
    if kubectl get namespace "$NAMESPACE" &>/dev/null; then
      ...
      # Redis secret check
      log_info "Checking Redis secret..."

      if kubectl get secret redis-credentials -n "$NAMESPACE" &>/dev/null; then
        REDIS_PASSWORD=$(kubectl get secret redis-credentials -n "$NAMESPACE" -o jsonpath='{.data.redis-password}' 2>/dev/null)
        if [[ -n "$REDIS_PASSWORD" ]]; then
          log_pass "Secret 'redis-credentials' exists with required key (redis-password)"
        else
          log_fail "Secret 'redis-credentials' exists but missing key: redis-password"
          log_info "Recreate with: kubectl create secret generic redis-credentials \\"
          log_info "  --from-literal=redis-password='<redis-password>' \\"
          log_info "  -n ${NAMESPACE}"
        fi
      else
        log_fail "Secret 'redis-credentials' not found — create before deployment"
        log_info "Create with: kubectl create secret generic redis-credentials \\"
        log_info "  --from-literal=redis-password='<redis-password>' \\"
        log_info "  -n ${NAMESPACE}"
      fi
